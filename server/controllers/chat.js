...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  emitEvent(
    ...
    ALERT,
    chat.members,
    `${allUsersName} has been added in the ${chat.name} group`
  );
...
const removeMembers = TryCatch(async (req, res, next) => {
  ...
  emitEvent(req, ALERT, chat.members, {
    message: `${userThatWillBeRemoved.name} has been removed from the ${chat.name} group`,
    chatId,
  });
...
console.log("Hellow");
  ...
  if (files.length < 1)
    return next(new ErrorHandler("Please Provided Attachments", 400));
